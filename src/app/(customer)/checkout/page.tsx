  ...
  useEffect(() => {
    const loadShop = async () => {
      try {
        ...
        const found = shops.find((s) => s.name === cart.items[0].shop_name);
        if (found) {
          setShop(found);
          //-------------------------add start-------------------------
          // if shop has no delivery, force pickup
          if (!found.has_delivery) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        notes:
          fulfillment === "pickup" ? `Pickup order. ${form.notes}` : form.notes,
      };
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-2xl mx-auto px-4 py-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {/*-------------------------add start-------------------------*/}
          {/* Show delivery/pickup toggle ONLY if shop offers delivery */}
          {shop?.has_delivery && (
              ...
              <div className="flex gap-3">
                ...
                >
                  ...
                  <p className="text-xs font-normal mt-0.5 opacity-70">
                    Rs.{shop.delivery_charge} · Delivered to your address
                  </p>
                </button>
                <button
                  type="button"
          ...
          {/* Pickup only notice — shown when shop has no delivery */}
          {shop && !shop.has_delivery && (
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
              <p className="text-sm font-medium text-amber-800">
                This shop does not offer delivery
              </p>
              <p className="text-xs text-amber-600 mt-1">
                You can pick up your order from: {shop.address}
              </p>
            </div>
          )}
          {/*-------------------------add end-------------------------*/}
          ...
          {/*-------------------------add start-------------------------*/}
          {/* Order summary — shows fulfillment type and fee */}
          {shop && (
            <div className="bg-white rounded-xl border border-gray-200 p-5">
              <h2 className="font-semibold text-gray-900 mb-3">
                Order summary

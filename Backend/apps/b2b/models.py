...
class ColdStorageProduct(models.Model):
    ...
    min_order_kg = models.DecimalField(
        max_digits=8, decimal_places=2, default=10, null=True, blank=True
    )
...
class B2BOrder(models.Model):
    ...
    shop         = models.ForeignKey(Shop, on_delete=models.CASCADE, related_name="b2b_orders")
    cold_storage = models.ForeignKey(ColdStorage, on_delete=models.CASCADE, related_name="incoming_orders")
    ...
    order_source  = models.CharField(max_length=10, choices=OrderSource.choices, default="app")
...
class B2BOrderItem(models.Model):
    ...
    product = models.ForeignKey(ColdStorageProduct, on_delete=models.PROTECT, related_name="order_items")

    product_name_snapshot = models.CharField(max_length=255)
    unit_type             = models.CharField(max_length=10, choices=UnitType.choices, default="kg")
    ...
    # Piece orders: filled after weighing
    ...
    def save(self, *args, **kwargs):
        if self.unit_type == "kg" and self.price_per_kg_snapshot and self.quantity:
            self.quantity_kg = self.quantity
            self.line_total  = self.quantity * self.price_per_kg_snapshot
            self.price       = self.line_total
...
class B2BOrderStatusHistory(models.Model):
    order       = models.ForeignKey(B2BOrder, on_delete=models.CASCADE, related_name="history")
...
class LedgerEntry(models.Model):
    ...
    # Who collected the cash payment
    ...
    # Who physically weighed and priced the order
